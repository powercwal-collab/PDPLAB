...
| 诊断进度 | 后端任务完成 | 评分确认 | `diagnosisJobId`, 初评结果 |
| 评分确认 | 人工确认并锁定 | 项目总览 | `diagnosisVersionId` |
...
当前本地版本使用定时 mock 演示四阶段进度。正式后端接入后应改为异步任务：

1. 上传文件后创建 `PdpSource`。
2. 调用 `POST /diagnosis-jobs` 创建诊断任务。
3. 前端保存 `diagnosisJobId` 并进入诊断进度页。
4. 前端使用 SSE、WebSocket 或短轮询获取任务状态。
5. 后端按阶段返回 `queued / processing / completed / failed`。
6. 完成后返回初评结果，但不自动锁定评分。
7. 用户进入评分确认页，检查 11 模块证据后生成正式评分版本。
